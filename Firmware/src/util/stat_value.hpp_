...
public:
	...
	StatValue( T1 m=0, T2 sum=0, T4 sumsq=0 ) : n(m), sx(sum), sx2(sumsq)		{}
	...
	void Accumulate( StatValue<T1,T2,T4>& v )
	{
		this->n += v.n;
		this->sx += v.sx;
		this->sx2 += v.sx2;
	...
	constexpr uint8_t	Size() const 	{ return (sizeof( T1 ) + sizeof( T2 ) + sizeof( T4 )); }
...
typedef StatValue< uint16_t, uint32_t, uint64_t>	StatValue12;
typedef StatValue< uint16_t, uint32_t, uint64_t>	StatValue16;
typedef StatValue< uint32_t, uint64_t, uint64_t>	StatValue24;	// max no of square smpls = 65536
